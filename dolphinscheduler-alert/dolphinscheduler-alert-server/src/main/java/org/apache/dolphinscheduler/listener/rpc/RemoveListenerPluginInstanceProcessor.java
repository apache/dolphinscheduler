...
@Component
@Slf4j
public final class RemoveListenerPluginInstanceProcessor implements NettyRequestProcessor {

    private final ListenerPluginService listenerPluginService;

    public RemoveListenerPluginInstanceProcessor(ListenerPluginService listenerPluginService) {
        this.listenerPluginService = listenerPluginService;
    }

    @Override
    public void process(Channel channel, Message message) {
        RemoveListenerPluginInstanceRequest request =
                JsonSerializer.deserialize(message.getBody(), RemoveListenerPluginInstanceRequest.class);

        log.info("Received command : {}", request);

        ListenerResponse listenerResponse = listenerPluginService.removeListenerInstance(
