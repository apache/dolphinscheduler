...
@Component
@Slf4j
public final class UpdateListenerPluginInstanceProcessor implements NettyRequestProcessor {

    private final ListenerPluginService listenerPluginService;

    public UpdateListenerPluginInstanceProcessor(ListenerPluginService listenerPluginService) {
    ...
    public void process(Channel channel, Message message) {
        UpdateListenerPluginInstanceRequest request =
                JsonSerializer.deserialize(message.getBody(), UpdateListenerPluginInstanceRequest.class);

        log.info("Received command : {}", request);

        ListenerResponse listenerResponse = listenerPluginService.updateListenerInstance(
