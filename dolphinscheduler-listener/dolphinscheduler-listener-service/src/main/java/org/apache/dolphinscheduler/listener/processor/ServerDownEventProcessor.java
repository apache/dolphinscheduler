...
public class ServerDownEventProcessor implements ListenerEventProcessor {
    ...
    @Override
    public void process(ListenerPlugin plugin, JdbcListenerEvent event) {
        ServerDownListenerEvent dsListenerEvent =
                JSONUtils.parseObject(event.getContent(), ServerDownListenerEvent.class);
        dsListenerEvent.setListenerInstanceParams(event.getParams());
        plugin.onServerDown(dsListenerEvent);
    }
}
