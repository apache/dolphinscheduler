...
public class ProcessUtils {
    ...
    public static void kill(TaskInstance taskInstance) {
        try {
            ...
            if (processId == 0) {
                logger.error("process kill failed, process id :{}, task id:{}", processId, taskInstance.getId());
                return;
            }
    ...
    /**
     * kill spark task by call restful api of yarn
     * @param taskInstance task instance
     */
    public static void killSparkTask(TaskInstance taskInstance) throws InterruptedException {
        try {
            String state = "{\"state\": \"KILLED\"}";

            List<String> appIds = getYarnAppIds(taskInstance);
            String appId = appIds.get(appIds.size() - 1);

            String appAddress = HadoopUtils.getYarnAppAddress();
    ...
    /**
     * get yarn task app ids
     * @param taskInstance task instance
     * @return yarn task app ids
     * @throws InterruptedException
     */
    private static List<String> getYarnAppIds(TaskInstance taskInstance) throws InterruptedException {
