...
public class HostUpdateProcessor implements NettyRequestProcessor {
    ...
    public void process(Channel channel, Command command) {
        Preconditions.checkArgument(CommandType.PROCESS_HOST_UPDATE == command.getType(), String.format("invalid command type : %s", command.getType()));
        ProcessHostUpdateCommand updateCommand = JSONUtils.parseObject(command.getBody(), ProcessHostUpdateCommand.class);
