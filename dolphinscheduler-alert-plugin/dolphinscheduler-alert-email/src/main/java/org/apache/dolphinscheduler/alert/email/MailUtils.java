...
import org.apache.dolphinscheduler.alert.email.template.AlertTemplate;
import org.apache.dolphinscheduler.alert.email.template.DefaultHTMLTemplate;
...
public class MailUtils {
    ...
    public MailUtils(Map<String, String> config)
    {
        ...
        starttlsEnable = config.get(Constants.MAIL_SMTP_STARTTLS_ENABLE);
        requireNonNull(starttlsEnable, Constants.MAIL_SMTP_STARTTLS_ENABLE + " must not null");

        sslEnable = config.get(Constants.MAIL_SMTP_SSL_ENABLE);
        requireNonNull(sslEnable, Constants.MAIL_SMTP_SSL_ENABLE + " must not null");

        sslTrust = config.get(Constants.MAIL_SMTP_SSL_TRUST);
        requireNonNull(sslTrust, Constants.MAIL_SMTP_SSL_TRUST + " must not null");
