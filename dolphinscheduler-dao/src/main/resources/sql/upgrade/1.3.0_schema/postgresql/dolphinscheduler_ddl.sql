...
d//
CREATE
OR REPLACE FUNCTION uc_dolphin_T_t_ds_process_definition_A_modify_by() RETURNS void AS $$
BEGIN
       IF
...
d//
CREATE
OR REPLACE FUNCTION uc_dolphin_T_t_ds_task_instance_A_executor_id() RETURNS void AS $$
BEGIN
       IF
NOT EXISTS (SELECT 1 FROM information_schema.COLUMNS
...
d//
CREATE
OR REPLACE FUNCTION uc_dolphin_T_t_ds_task_instance_C_app_link() RETURNS void AS $$
BEGIN
       IF
...
d//
CREATE
OR REPLACE FUNCTION uc_dolphin_T_t_ds_process_definition_A_process_definition_unique() RETURNS void AS $$
BEGIN
       IF
NOT EXISTS (SELECT 1 FROM pg_stat_all_indexes
