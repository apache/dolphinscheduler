...
public enum Status {
    ...
    TASK_GROUP_NAME_EXSIT(110014,"this task group name is repeated in a project","该任务组名称在一个项目中已经使用"),
    TASK_GROUP_SIZE_ERROR(110015,"task group size error","任务组大小应该为大于1的整数"),
    TASK_GROUP_STATUS_ERROR(110016,"task group status error","任务组正在被使用或已经被关闭"),
    TASK_GROUP_FULL(110017,"task group is full","任务组已经满了"),
    TASK_GROUP_USED_SIZE_ERROR(110018,"the used size number of task group is dirty","任务组使用的容量发生了变化"),
    TASK_GROUP_QUEUE_RELEASE_ERROR(110019,"relase task group queue failed","任务组资源释放时出现了错误"),
    TASK_GROUP_QUEUE_AWAKE_ERROR(110020,"awake waiting task failed","任务组使唤醒等待任务时发生了错误"),
    CREATE_TASK_GROUP_ERROR(110022,"create task group error","创建任务组错误"),
    UPDATE_TASK_GROUP_ERROR(110023,"update task group list error","更新任务组错误"),
    QUERY_TASK_GROUP_LIST_ERROR(110024,"query task group list error","查询任务组列表错误"),
    CLOSE_TASK_GROUP_ERROR(110025,"close task group error","关闭任务组错误"),
    START_TASK_GROUP_ERROR (110026,"start task group error","启动任务组错误"),
    QUERY_TASK_GROUP_QUEUE_LIST_ERROR(110027,"query task group queue list error","查询任务组队列列表错误"),
    TASK_GROUP_CACHE_START_FAILED(110028,"cache start failed","任务组相关的缓存启动失败");
