...
public enum FuncPermissionEnum{
    ...
    CREATE_TOKEN("安全中心", "Security", "创建令牌", "Create Token", "security:token:create", 4),
    ...
    GENERATE_TOKEN("安全中心", "Security", "生成令牌", "Generate Token", "security:token:generate", 4),
    ...
    USER_REVOKE_PROJECT ("安全中心", "Security", "撤销项目", "Revoke", "security:user:revoke:project", 4),
    USER_GRANT_K8SNAMESPACE ("安全中心", "Security", "授权k8s命名空间", "Grant", "security:user:grant:k8snamespace", 4),
    USER_GRANT_PROJECT ("安全中心", "Security", "授权项目", "Grant", "security:user:grant:project", 4),
    ...
    VERSION_DELETE("项目管理", "Project", "查看版本信息", "Version delete", "project:version:delete", 4)
            ;
    ...
    private final static Map<ExecuteType,String> map = new HashMap<>();

    static{
        for(ExecuteType type : ExecuteType.values()){
            switch (type){
                case REPEAT_RUNNING:
                    map.put(type,RERUN.toString());
                    break;
                case RECOVER_SUSPENDED_PROCESS:
                    map.put(type,RECOVERY_SUSPEND.toString());
                    break;
                case START_FAILURE_TASK_PROCESS:
                    map.put(type,FAILED_TO_RETRY.toString());
                    break;
                case STOP:
                    map.put(type,STOP.toString());
                    break;
                case PAUSE:
                    map.put(type,PAUSE.toString());
                    break;
                case NONE:
                    map.put(type,null);
                    break;
    ...
    public static String of(ExecuteType type) {
        return map.get(type);
    }
