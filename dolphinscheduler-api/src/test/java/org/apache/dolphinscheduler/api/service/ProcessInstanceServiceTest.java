...
public class ProcessInstanceServiceTest {
    ...
    public void testQueryProcessInstanceList() {
        ...
        when(projectService.checkProjectAndAuth(loginUser, project, projectCode, FuncPermissionEnum.WORKFLOW_INSTANCE.toString())).thenReturn(result);
    ...
    public void testUpdateProcessInstance() {
        ...
        //project auth fail
        when(projectMapper.queryByCode(projectCode)).thenReturn(project);
        when(projectService.checkProjectAndAuth(loginUser, project, projectCode,FuncPermissionEnum.UPDATE.toString())).thenReturn(result);
    ...
    public void testDeleteProcessInstanceById() {
        ...
        putMsg(result, Status.SUCCESS, projectCode);
        when(projectMapper.queryByCode(projectCode)).thenReturn(project);
        when(projectService.checkProjectAndAuth(loginUser, project, projectCode,FuncPermissionEnum.DELETE.toString())).thenReturn(result);
        when(processService.findProcessInstanceDetailById(1)).thenReturn(null);
    }
