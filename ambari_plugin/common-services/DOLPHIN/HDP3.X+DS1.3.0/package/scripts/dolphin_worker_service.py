...
class DolphinWorkerService(Script):
  ...
  def start(self, env):
    ...
    env.set_params(params)
    self.configure(env)
    no_op_test = format("ls {dolphin_pidfile_dir}/dolphinscheduler-worker-server.pid >/dev/null 2>&1 && ps `cat {dolphin_pidfile_dir}/dolphinscheduler-worker-server.pid` | grep `cat {dolphin_pidfile_dir}/dolphinscheduler-worker-server.pid` >/dev/null 2>&1")
