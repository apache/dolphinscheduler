...
public class ProcessService {
    ...
    private ProcessInstance constructProcessInstance(Command command, String host) {
        ...
        if (cmdParam != null) {
            ...
            if (cmdParam.containsKey(CMDPARAM_RECOVER_PROCESS_ID_STRING)) {
                String processId = cmdParam.get(CMDPARAM_RECOVER_PROCESS_ID_STRING);
                processInstanceId = Integer.parseInt(processId);
                if (processInstanceId == 0) {
                    logger.error("command parameter is error, [ ProcessInstanceId ] is 0");
                    return null;
                }
            } else if (cmdParam.containsKey(CMDPARAM_SUB_PROCESS)) {
                // sub process map
                String pId = cmdParam.get(CMDPARAM_SUB_PROCESS);
            ...
            if (cmdParam.containsKey(CMDPARAM_SUB_PROCESS)) {
                processInstance.setCommandParam(command.getCommandParam());
            }
        } else {
    ...
    public Command createSubProcessCommand(ProcessInstance parentProcessInstance,
        ...
        Integer childDefineId = Integer.parseInt(subProcessParam.get(CMDPARAM_SUB_PROCESS_DEFINE_ID));
    ...
    public void processNeedFailoverProcessInstances(ProcessInstance processInstance) {
        ...
        cmd.setCommandParam(String.format("{\"%s\":%d}", CMDPARAM_RECOVER_PROCESS_ID_STRING, processInstance.getId()));
