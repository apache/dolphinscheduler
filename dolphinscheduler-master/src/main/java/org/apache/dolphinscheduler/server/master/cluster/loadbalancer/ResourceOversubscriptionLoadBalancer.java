...
public class ResourceOversubscriptionLoadBalancer implements IWorkerLoadBalancer {
    ...
    public ResourceOversubscriptionLoadBalancer(WorkerClusters workerClusters,
    ...
    public Optional<String> select(@NotNull String workerGroup) {
        List<WeightedServer<WorkerServerMetadata>> weightedServers =
                workerClusters.getNormalWorkerServerAddressByGroup(workerGroup)
                        ...
                        .filter(ws -> ws.getServer().getMemoryUsage() < 0.80)
    ...
    public WorkerLoadBalancerType getType() {
        return WorkerLoadBalancerType.RESOURCE_OVERSUBSCRIPTION;
    }
}
