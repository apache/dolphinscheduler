  ...
  export default {
    ...
    methods: {
      ...
      bindGraphEvent () {
        ...
        // right click
        this.graph.on('node:contextmenu', ({ e, x, y, cell }) => {
        ...
        this.graph.on('edge:connected', ({ isNew, edge }) => {
          ...
          if (!this.dagChart.edgeIsValid(edgeData)) {
            edge.remove()
          }
        })
      },
      ...
      setNodeHighlight (node) {
        ...
        node.setPortProp(X6_PORT_NAME, 'attrs', PORT_HIGHLIGHT_PROPS.attrs)
      },
      /**
       * Reset node style
       * @param {Node} node
       */
      resetNodeStyle (node) {
        const url = require(`../images/task-icos/${node.data.taskType.toLocaleLowerCase()}.svg`)
        node.setAttrs(NODE_PROPS.attrs)
        node.setAttrByPath('image/xlink:href', url)
        node.setPortProp(
          X6_PORT_NAME,
          'attrs',
          PORT_PROPS.groups[X6_PORT_NAME].attrs
        )
      },
      /**
       * Set edge highlight
      ...
      calcNodeCoordinate (mClientX, mClientY) {
        // Distance from the mouse to the top-left corner of the container;
        const { left: cX, top: cY } = this.$refs.container.getBoundingClientRect()
      ...
      _onDrop (e) {
        ...
        const { x, y } = this.calcNodeCoordinate(e.clientX, e.clientY)
