...
<template>
  <div class="dag-canvas">
    ...
    >
      ...
      <div ref="minimap" class="minimap" style="opacity: 0"></div>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.$nextTick(() => {
        document.querySelector('.x6-graph-scroller').style.opacity = 0
        const div = document.createElement('div')
        div.className = 'task-table'
        div.style.cssText = 'position: absolute; top: 0; bottom: 0; left: 0; right: 0'
        const targetDiv = document.querySelector('.dag-container')
    ...
    methods: {
      ...
      setNodeName (id, name) {
        ...
        if (node) {
          ...
          <div class="item-cell">${$t('Type')}：${node.data.taskType}</div>
          `
