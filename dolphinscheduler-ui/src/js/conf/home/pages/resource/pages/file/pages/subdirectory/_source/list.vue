        ...
        <el-table-column :label="$t('Operation')" width="190">
          <template slot-scope="scope">
            ...
            <el-tooltip :content="$t('ReUpload File')" placement="top" :enterable="false">
              <span><el-button type="primary" size="mini" icon="el-icon-upload" @click="_childReUpload(scope.row)" :disabled="scope.row.directory? true: false" circle></el-button></span>
            </el-tooltip>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      _childReUpload (item) {
        findComponentDownward(this.$root, 'roof-nav')._fileChildReUpload('FILE', item)
      },
      _downloadFile (item) {
