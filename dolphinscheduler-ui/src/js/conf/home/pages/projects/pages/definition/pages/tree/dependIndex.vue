 ...
 */
<template>
  <m-list-construction :title="$t('DependView')">
...
<script>
  ...
  export default {
    ...
    props: {},
    methods: {
      ...mapActions('dag', ['getViewTreeByDepend']),
      /**
       * get tree data
       */
      _getViewTree () {
      ...
      _rtTasksDag () {
        ...
        this.$router.push({ path: `/projects/definition/depend/tree/${id}`, query: query })
      },
      /**
      ...
      _subProcessHandle (subProcessId) {
        ...
        this.$router.push({ path: `/projects/definition/depend/tree/${subProcessId}`, query: { subProcessIds: subProcessIds.join(',') } })
      },
      _onChangeSelect (o) {
        this._getViewTree()
      }
