...
export default {
  ...
  workflow: {
    ...
    crontab: 'Crontab',
    trigger_type: '触发类型',
    cron_trigger: 'Cron 表达式',
    interval_trigger: '固定间隔',
    interval: '间隔配置',
    hours: '小时',
    minutes: '分钟',
    seconds: '秒',
    repeat: '重复次数',
    unlimited_repeat_tip: '-1 表示不限次数',
    interval_must_be_positive: '固定间隔必须大于 0',
    misfire_policy: '错过触发策略',
    do_nothing: '不执行补偿，等待下一次调度',
    fire_and_proceed: '立即执行错过的任务，之后按正常节奏继续调度',
    ignore_misfires: '补齐所有错过的任务，之后按正常节奏继续调度',
    delete_confirm: '确定删除吗?',
