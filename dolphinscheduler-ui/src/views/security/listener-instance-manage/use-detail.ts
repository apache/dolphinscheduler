...
import {
  createListeneInstance,
  updateListenerInstance,
  verifListenerInstanceName
  ...
  const createOrUpdate = async (currentRecord: IRecord, json?: IJsonItem[]) => {
    ...
    try {
      if (currentRecord?.instanceName !== values.instanceName) {
        await verifListenerInstanceName({
          instanceName: values.instanceName
        })
      }
      const formData = new FormData()
      formData.append('instanceName', values.instanceName)
      formData.append('instanceParams', formatParams(json, values))
      formData.append('pluginDefineId', values.pluginDefineId)
      formData.append('listenerEventTypes', values.listenerEventTypes)
      if (currentRecord && currentRecord.id) {
        await updateListenerInstance(
          formData as any,
          currentRecord.id,
        )
      } else {
        await createListeneInstance(formData as any)
      }
      status.saving = false
      return true
    } catch (err) {
