...
public class XuguDataSourceProcessor extends AbstractDataSourceProcessor {
    ...
    public BaseConnectionParam createConnectionParams(BaseDataSourceParamDTO dataSourceParam) {
        XuguDataSourceParamDTO xuguDatasourceParam = (XuguDataSourceParamDTO) dataSourceParam;
        String address = String
                .format("%s%s:%s", DataSourceConstants.JDBC_XUGU, xuguDatasourceParam.getHost(),
                        xuguDatasourceParam.getPort());
        String jdbcUrl = StringUtils.isEmpty(xuguDatasourceParam.getDatabase()) ? address
                : String.format("%s/%s", address,
                        xuguDatasourceParam.getDatabase());

        XuguConnectionParam xuguConnectionParam = new XuguConnectionParam();
        xuguConnectionParam.setJdbcUrl(jdbcUrl);
        xuguConnectionParam.setDatabase(xuguConnectionParam.getDatabase());
        xuguConnectionParam.setAddress(address);
        xuguConnectionParam.setUser(xuguDatasourceParam.getUserName());
        xuguConnectionParam.setPassword(PasswordUtils.encodePassword(xuguDatasourceParam.getPassword()));
        xuguConnectionParam.setDriverClassName(getDatasourceDriver());
        xuguConnectionParam.setValidationQuery(getValidationQuery());
        xuguConnectionParam.setOther(xuguDatasourceParam.getOther());
    ...
    public Connection getConnection(ConnectionParam connectionParam) throws ClassNotFoundException, SQLException {
        ...
        return DriverManager.getConnection(getJdbcUrl(xuguConnectionParam), xuguConnectionParam.getUser(),
