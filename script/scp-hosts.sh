...
do
  ...
  echo "scp dirs to $host/$installPath starting"
	ssh -p $sshPort $host  "cd $installPath/; rm -rf bin/ conf/ lib/ script/ sql/ ui/ plugin/"

  for dsDir in bin master-server worker-server alert-server api-server logger-server ui plugin
  do
