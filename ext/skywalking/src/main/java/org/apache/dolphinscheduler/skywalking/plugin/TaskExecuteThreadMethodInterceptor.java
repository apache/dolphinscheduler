...
public class TaskExecuteThreadMethodInterceptor implements InstanceMethodsAroundInterceptor {
    ...
    public void beforeMethod(EnhancedInstance objInst, Method method, Object[] allArguments, Class<?>[] argumentsTypes, MethodInterceptResult result) throws Throwable {
        ...
        String operationName = OPERATION_NAME_PREFIX
            + type.getDesc()
            + "/" + getProjectId(executionContext.getProjectCode())
            + "/" + getProcessDefinitionId(executionContext.getProcessDefineCode())
            + "/" + executionContext.getTaskName();

        AbstractSpan span = ContextManager.createLocalSpan(operationName);
        span.setComponent(Utils.DOLPHIN_SCHEDULER);

        TAG_PROJECT_ID.set(span, String.valueOf(executionContext.getProjectCode()));
        TAG_PROCESS_DEFINITION_ID.set(span, String.valueOf(executionContext.getProcessDefineCode()));
