...
| worker.server-load-protection.max-data-basedir-disk-usage-percentage-thresholds | 0.8       | worker最大data.basedir.path目录磁盘使用值,只有当前data.basedir.path目录磁盘使用值低于该值,worker服务才能接收任务. 默认值为0.8: 会使用80%的data.basedir.path目录磁盘空间 |
