...
# 前言
本文档为dolphinscheduler配置文件说明文档,针对版本为 dolphinscheduler-3.0.0-beta-2 版本.
...
## 2.数据库连接相关配置
在DS中使用Spring Hikari对数据库连接进行管理，配置文件位置：
...
## 3.Zookeeper相关配置
DS使用Zookeeper进行集群管理、容错、事件监听等功能，配置文件位置：
|服务名称| 配置文件 |
|--|--|
Master Server | `master-server/conf/application.yaml`
Api Server| `api-server/conf/application.yaml`
...
## 10.dolphinscheduler_env.sh [环境变量配置]
