...
* 为 `t_ds_alert` 表的 `(sign, workflow_instance_id, alert_type)` 新增唯一约束 `uk_alert_dedup`。升级脚本不会删除任何已有数据；如果已存在重复行，索引创建将失败，需要由用户自行处理。任务结果告警由应用代码插入，重复插入由该约束拒绝，DAO 层将产生的 `DuplicateKeyException` 视为跳过，从而确保任务结果告警在并发投递时幂等。([#18549](https://github.com/apache/dolphinscheduler/pull/18549))
