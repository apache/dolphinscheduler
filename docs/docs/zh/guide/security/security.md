...
DolphinScheduler 租户是 Worker 执行任务所使用的 Linux 执行身份，不应将其视为外部数据系统的细粒度权限主体。

- 外部数据系统（数据库、对象存储、数据目录等）负责验证和执行自身的数据权限。仅拥有 DolphinScheduler 租户身份不足以获得对外部资源的访问权限。
- 长期有效的外部凭据（密码、API 密钥、令牌等）不应存储在任务定义中或打印在日志里。建议使用 DolphinScheduler 内置的凭据管理功能或外部密钥管理方案。
- 配置数据源连接或资源访问时，请始终确认外部系统的访问控制策略与您组织的安全要求一致。
