...
> **当前状态（重要）：** 上下游任务间的 **FILE 文件参数传递在当前版本（含 3.4.x）不可用**。
>
> 在 varPool 重构之后，运行时的上传/下载链路（`TaskFilesTransferUtils` / 资源中心 `DATA_TRANSFER` 中转）已不再接入。配置类型为 `FILE` 的自定义参数 **不会** 将文件传递给下游任务。
>
> 跟踪 Issue：[#18166](https://github.com/apache/dolphinscheduler/issues/18166)。当前参数传递仅支持数据值；文件传递功能待重新设计/恢复。
>
> 下文描述的是 **历史行为**，在功能恢复前仅供参考。请勿在当前版本依赖文中的 Demo 工作流。
...
- 功能可用时，上下游任务间的文件传递基于资源中心作为中转，数据保存在资源中心`DATA_TRANSFER`的目录下, 因此**必须开启资源中心功能**，详情请参考[资源中心配置详情](../resource/configuration.md), 否则无法使用文件参数功能。
...
> 下列 Demo 对应历史 FILE 参数设计。在当前版本中不会通过 `DATA_TRANSFER` 上传/下载文件，下游任务无法收到文件。
