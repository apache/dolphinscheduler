...
因为参数的值存在多个来源，当参数名相同时，就需要会存在参数优先级的问题。DolphinScheduler 参数的优先级从高到低为：`上游任务传递的参数 > 启动参数 > 本地参数 > 全局参数 > 项目级别参数 > 内置参数`
...
其中节点【useParam】在使用变量时直接使用即可。另外节点【createParam】设置了 "key" 和 "key1" 两个变量，这里用户定义了一个与上游节点传递的变量名相同的变量 key，并且赋值为 "12"，但是由于我们设置的优先级的关系，这里的值 "12" 会被抛弃，最终上游节点设置的变量值会被使用。
...
这里的 "id" 是上游节点设置的变量，用户在节点【useParam1】、节点【useParam2】中设置了相同参数名 "id" 的参数。而节点【use_param】中使用了最先结束的【useParam1】的值。
