...
|IP/호스트 이름 |Spark 서비스 IP를 입력하세요.|
|포트 |Spark 서비스 포트를 입력하세요.|
|사용자 이름 |Spark 연결을 위한 사용자 이름을 설정합니다.|
|비밀번호 |Spark 연결을 위한 비밀번호를 설정하세요.|
|데이터베이스 이름 |Spark 연결의 데이터베이스 이름을 입력합니다.|
|Jdbc 연결 매개변수 |Spark 연결을 위한 매개변수 설정(JSON 형식)|
