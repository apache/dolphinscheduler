...
**Workflow definition**: Visualization **DAG** by dragging task nodes and establishing associations of task nodes

**Workflow instance**: A workflow instance is an instantiation of a workflow definition, which can be generated by manual startup or  scheduling. The workflow definition runs once, a new workflow instance is generated

**Task instance**: A task instance is the instantiation of a specific task node when a workflow instance runs, which indicates the specific task execution status

**Task type**: Currently supports SHELL, SQL, SUB_PROCESS (sub-process), PROCEDURE, MR, SPARK, PYTHON, DEPENDENT (dependency), and plans to support dynamic plug-in extension, note: the sub-**SUB_PROCESS** is also A separate workflow definition that can be launched separately

**Schedule mode** :  The system supports timing schedule and manual schedule based on cron expressions. Command type support: start workflow, start execution from current node, resume fault-tolerant workflow, resume pause workflow, start execution from failed node, complement, timer, rerun, pause, stop, resume waiting thread. Where **recovers the fault-tolerant workflow** and **restores the waiting thread** The two command types are used by the scheduling internal control and cannot be called externally
...
**Priority**: Supports the priority of workflow instances and task instances. If the workflow instance and task instance priority are not set, the default is first in, first out.

**Mail Alert**: Support **SQL Task** Query Result Email Send, Workflow Instance Run Result Email Alert and Fault Tolerant Alert Notification

**Failure policy**: For tasks running in parallel, if there are tasks that fail, two failure policy processing methods are provided. **Continue** means that the status of the task is run in parallel until the end of the workflow failure. **End** means that once a failed task is found, Kill also drops the running parallel task and the workflow ends.
...
1. The core workflow algorithm for obtaining distributed locks is as follows

 <p align="center">
   <img src="../../../img/architecture-design/distributed_lock.png" alt="Get Distributed Lock Workflow" width="70%" />
 </p>

2. Scheduler thread distributed lock implementation flow chart in DolphinScheduler:

 <p align="center">
   <img src="../../../img/architecture-design/distributed_lock_procss.png" alt="Get Distributed Lock Workflow" />
 </p>

##### Third, the thread is insufficient loop waiting problem

- If there is no subprocess in a DAG, if the number of data in the Command is greater than the threshold set by the thread pool, the direct workflow waits or fails.
...
In the above figure, MainFlowThread waits for SubFlowThread1 to end, SubFlowThread1 waits for SubFlowThread2 to end, SubFlowThread2 waits for SubFlowThread3 to end, and SubFlowThread3 waits for a new thread in the thread pool, then the entire DAG workflow cannot end, and thus the thread cannot be released. This forms the state of the child parent workflow loop waiting. At this point, the scheduling cluster will no longer be available unless a new Master is started to add threads to break such a "stuck."
...
1. Calculate the sum of the threads of all Masters, and then calculate the number of threads required for each DAG, that is, pre-calculate before the DAG workflow is executed. Because it is a multi-master thread pool, the total number of threads is unlikely to be obtained in real time.
2. Judge the single master thread pool. If the thread pool is full, let the thread fail directly.
3. Add a Command type with insufficient resources. If the thread pool is insufficient, the main workflow will be suspended. This way, the thread pool has a new thread, which can make the workflow with insufficient resources hang up and wake up again.
...
The Master monitors the directories of other Masters and Workers. If the remove event is detected, the workflow instance is fault-tolerant or the task instance is fault-tolerant according to the specific business logic.
...
Here we must first distinguish between the concept of task failure retry, workflow failure recovery, and workflow failure rerun:
...
- Workflow failure recovery is workflow level, is done manually, recovery can only be performed **from the failed node** or **from the current node**
- Workflow failure rerun is also workflow level, is done manually, rerun is from the start node
...
- There is also a logical node, which does not do the actual script or statement processing, but the logical processing of the entire workflow flow, such as sub-flow sections.
...
If there is a task failure in the workflow that reaches the maximum number of retries, the workflow will fail to stop, and the failed workflow can be manually rerun or workflow resumed.

##### V. Task priority design

In the early scheduling design, if there is no priority design and fair scheduling design, it will encounter the situation that the task submitted first may be completed simultaneously with the task submitted subsequently, but the priority of the workflow or task cannot be set. We have redesigned this, and we are currently designing it as follows:

- According to **different workflow instance priority** prioritizes **same workflow instance priority** prioritizes **task priority within the same workflow** takes precedence over **same workflow** commit order from high Go to low for task processing.
  - The specific implementation is to resolve the priority according to the json of the task instance, and then save the **workflow instance priority _ workflow instance id_task priority _ task id** information in the ZooKeeper task queue, when obtained from the task queue, Through string comparison, you can get the task that needs to be executed first.
    - The priority of the workflow definition is that some processes need to be processed before other processes. This can be configured at the start of the workflow or at the time of scheduled start. There are 5 levels, followed by HIGHEST, HIGH, MEDIUM, LOW, and LOWEST. As shown below

      <p align="center">
         <img src="../../../img/architecture-design/process_priority.png" alt="Workflow Priority Configuration" width="40%" />
       </p>
