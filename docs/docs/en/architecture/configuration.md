...
| task-log-archive.enabled                                                    | false                        | Whether to archive local task instance logs. When false the archiver never runs and no log file is touched.                                                                                  |
| task-log-archive.check-interval                                             | 1d                           | How often the archiver scans the log base directory for partitions to archive or expire.                                                                                                     |
| task-log-archive.archive-threshold                                          | 30d                          | A daily log partition is packed into a zip once it is older than this (measured in whole days from today).                                                                                    |
| task-log-archive.expire-enabled                                             | false                        | Whether archived zips are eventually deleted. This is the only setting that removes log data permanently.                                                                                     |
| task-log-archive.expire-threshold                                           | 90d                          | An archived zip is removed once it is older than this. Only consulted when `task-log-archive.expire-enabled` is true, and should exceed `task-log-archive.archive-threshold`.                 |
