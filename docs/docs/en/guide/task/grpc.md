...
|------------------------|---------------------------------------------------------------------------------------|
| Request Address        | gRPC request URL, must use the `hostname:port` format                                 |
| gRPC Credential Type   | Supports None (insecure), and client default SSL/TLS credential types                 |
| Protobuf Service Definition | Protobuf code defining the service in the `.proto` file, will be converted to JSON Descriptor when saved |
| Protobuf Service Definition JSON | Used for creating tasks via API, generated from the above definition as [JSON Descriptor](https://github.com/protobufjs/protobuf.js/#using-json-descriptors) |
| Request Method         | The rpc method to call, must be defined in the service definition, written as `Greeter/SayHello` format |
| Message Content        | Request message defined in JSON, will be merged into the service definition when requesting |
| Validation Condition   | Supports default gRPC status code (OK), custom status codes                           |
| Validation Content     | When validation condition is set to custom response code, must fill in validation content, must match [gRPC status codes](https://grpc.io/docs/guides/status-codes/) |
...
- Request Address: The address of the target gRPC service, here is the local 50051 port.
- gRPC Credential Type: Insecure
- Protobuf Definition: The protobuf definition used by the gRPC service
- Method Name: The rpc method to call, in `Greeter/SayHello` format
- Message Content: Request message defined in JSON
- Validation Condition: Default gRPC status code OK, custom status code
- Validation Content: When the validation condition is a custom status code, must fill in the validation content, which should exactly match the gRPC status code string
