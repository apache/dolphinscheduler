...
environment you used. The configuration files in directory `bin/dolphinscheduler_env.sh` and `bin/env/install_env.sh` and
`<api-server|alert-server|master-server|worker-server|tools>/bin/dolphinscheduler_env.sh`.
...
File `dolphinscheduler_env.sh` should be change in both directory `<bin|api-server|alert-server|master-server|worker-server|tools>`
...
Then, modify `tools/bin/dolphinscheduler_env.sh` to use mysql, change {user} and {password} to what you set in the previous step.
