...
public class HdfsFileConfigGenerator implements IConfigGenerator {

    private final SeatunnelParameters seatunnelParameters;

    public HdfsFileConfigGenerator(SeatunnelParameters seatunnelParameters) {
    ...
    public String createSourceConfig() {
        HdfsFileParameters hdfsParameters =
                JSONUtils.parseObject(seatunnelParameters.getSourceConfig(), HdfsFileParameters.class);

        Objects.requireNonNull(hdfsParameters);

        String configTemplate = initConfigTemplate();
        String fillConfigTemplate = String.format(configTemplate,
                hdfsParameters.getFilePath(),
                hdfsParameters.getFileFormat(),
                hdfsParameters.getDefaultFs());

        StringBuilder hdfsSourceSb = new StringBuilder("source {\n");
        hdfsSourceSb.append(fillConfigTemplate);

        List<Property> customParams = hdfsParameters.getCustomParams();
    ...
    public String createSinkConfig() {
        HdfsFileParameters hdfsParameters =
                JSONUtils.parseObject(seatunnelParameters.getTargetConfig(), HdfsFileParameters.class);
        Objects.requireNonNull(hdfsParameters);
