...
public class ConfigMapOperation implements AbstractK8sOperation {
    ...
    public ConfigMapOperation(KubernetesClient client) {
        this.client = client;
    }

    @Override
    public HasMetadata buildMetadata(YamlContent yamlContent) {
        return client.configMaps().load(yamlContent.getYaml()).get();
    }

    @Override
    public void createOrReplaceMetadata(HasMetadata metadata) {
        client.configMaps().createOrReplace((ConfigMap) metadata);
    }
