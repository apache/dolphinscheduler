 ...
 * flink args utils
 */
public class FlinkSqlArgsUtils {

    private FlinkSqlArgsUtils() {
     ...
     */
    public static List<String> buildArgs(FlinkSqlParameters param) {
        ...
        if (!LOCAL_DEPLOY_MODE.equals(deployMode)) {
            ...
            if (flinkVersion == null || FLINK_VERSION_BEFORE_1_10.equals(flinkVersion)) {
                ...
                if (taskManager > 0) {                        //-yn
                    args.add(FlinkSqlConstants.FLINK_TASK_MANAGE);
            ...
            if (StringUtils.isEmpty(others) || !others.contains(FlinkSqlConstants.FLINK_QUEUE)) {
...
//        }
        ProgramType programType = param.getProgramType();
        String flinksqlfile = param.getFlinksqlfile();
        if (programType != null && programType == ProgramType.SQL && StringUtils.isNotEmpty(flinksqlfile)){
            args.add(FlinkSqlConstants.FLINK_SQL_FILE);
            args.add(flinksqlfile);
